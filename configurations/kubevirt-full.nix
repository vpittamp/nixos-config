# KubeVirt Full Desktop Image - Complete Hetzner-equivalent with Home Manager
#
# This configuration builds a complete desktop VM image with:
# - Full system configuration (all modules from hetzner.nix)
# - Home Manager integration (vpittamp user with all customizations)
# - Performance optimizations for KubeVirt/TCG
# - RustDesk + Tailscale with automation
# - Pre-configured PWAs, Plasma settings, development tools
#
# Build Command:
#   nix build .#nixos-kubevirt-full-image
#
# The result will be a qcow2 image at: result/nixos.qcow2
#
...

{
  imports = [
    ...
    # All system modules from Hetzner config
    ...
    # KDE Performance optimizations for VM
  ...
  services.rustdesk = {
    enable = true;
    user = "vpittamp";  # Use vpittamp user for full home-manager integration
