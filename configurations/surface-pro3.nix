...
in
{
  ...
  # Wi-Fi power save off. The Marvell 88W8897 is already the least reliable part
  # of this machine -- its Bluetooth half wedges on its own (see
  # home-assistant-watchdog.nix) -- and this host is a headless always-on hub
  # where a few hundred mW of savings buys nothing and a deaf radio costs the
  # whole house its automation.
  networking.networkmanager.wifi.powersave = false;

  # Declarative Wi-Fi profile, mirroring configurations/surface.nix. The PSK is
  # NOT in this repo: it is substituted at runtime from
  # /etc/NetworkManager/secrets.env (root:root 0600, provisioned out-of-band).
  #
  # This was previously left stateful, on the reasoning that it kept the PSK out
  # of the repo. The env-file indirection already achieves that, and leaving it
  # stateful cost 18h43m of downtime on 2026-09-12: a WAN blip dropped the
  # association, NetworkManager fell through to a stale 'XFSETUP-37B4' profile
  # left over from install day (2026-08-07), failed it six times, declared the
  # activation failed, and never tried anything again. Two other strays --
  # 'iPhone (2)' and 'sprinkles' -- were sitting in the same directory as the
  # next versions of the same accident.
  #
  # autoconnect-priority is the direct fix for the fall-through: every profile
  # was tied at 0, so NetworkManager ordered them by timestamp and was free to
  # pick a dead setup SSID over the house network.
  networking.networkmanager.ensureProfiles = {
    environmentFiles = [ "/etc/NetworkManager/secrets.env" ];
    profiles."Linksys 416" = {
      connection = {
        id = "Linksys 416";
        # Reuses the UUID of the profile installed on 2026-08-07 so this
        # replaces it in place rather than racing a duplicate.
        uuid = "36f3e2d8-989b-4efe-9a98-09f3c37766f3";
        type = "wifi";
  ...
  # WAN blip into an 18-hour outage; nothing on a headless host notices or
  # retries. rebootAfter is on here specifically because this box runs the
  # house and has no local user to notice it is gone.
  services.wifiWatchdog = {
    enable = true;
    interface = "wlp1s0";
    connection = "Linksys 416";
