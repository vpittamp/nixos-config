...
case $MONITOR_COUNT in
    ...
    1)
        ...
        for ws in {1..9}; do
            "$I3MSG" "workspace number $ws; move workspace to output ${MONITORS[0]}" > /dev/null 2>&1 || true
        done
        ;;
    ...
    2)
        ...
        for ws in 1 2; do
            "$I3MSG" "workspace number $ws; move workspace to output $PRIMARY_MONITOR" > /dev/null 2>&1 || true
        done
        ...
        for ws in {3..9}; do
            "$I3MSG" "workspace number $ws; move workspace to output $SECONDARY_MONITOR" > /dev/null 2>&1 || true
        done
        ;;
    ...
    *)
        ...
        TERTIARY_MONITOR="${MONITORS[2]}"
        for ws in {6..9}; do
            "$I3MSG" "workspace number $ws; move workspace to output $TERTIARY_MONITOR" > /dev/null 2>&1 || true
        done
        ;;
esac
