...
async def main_async(args: argparse.Namespace) -> int:
    ...
    # Cross-host session aggregation now happens K8s-side via the
    # session-aggregator (PittampalliOrg/stacks images/session-aggregator/).
    # Each host's monitor only emits local session state.
    output = OutputWriter()
