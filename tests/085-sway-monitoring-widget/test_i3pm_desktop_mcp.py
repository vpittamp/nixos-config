    ...
    def request(self, method, params=None):
        ...
        if method == "runtime.snapshot":
            return {
                ...
                "active_outputs": ["DP-1"],
                "sessions": [{"session_key": "herdr:pane:1", "pane_id": "1", "source": "herdr"}],
                "scratchpad": {"available": True},
                "active_terminal": {},
                "launch_stats": {},
                "current_ai_session_key": "herdr:pane:1",
            }
...
def test_desktop_context_uses_runtime_snapshot_and_herdr_rows():
    client = FakeClient()

    result = i3pm_desktop_mcp.dispatch_tool(client, "get_desktop_context", {})

    assert client.calls == [("runtime.snapshot", {})]
...
def test_focus_window_uses_current_daemon_focus_endpoint():
    ...
    assert ("runtime.snapshot", {}) in client.calls
