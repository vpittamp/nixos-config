...
- Python 3.11+ (existing i3pm daemon runtime) + i3ipc-python (i3ipc.aio for async Sway IPC), asyncio, existing i3pm daemon infrastructure (057-env-window-matching)
- Python 3.11+ (existing daemon), TypeScript/Deno 1.40+ (CLI) + i3ipc-python (i3ipc.aio for async), asyncio, Pydantic (data validation), Deno standard library (@std/cli, @std/path) (058-python-backend-consolidation)
- JSON files (`~/.config/i3/projects/*.json`, `~/.config/i3/layouts/*.json`) (058-python-backend-consolidation)

## Recent Changes
- 058-python-backend-consolidation: Consolidated backend operations from TypeScript to Python daemon, eliminating code duplication (especially duplicate /proc environment reading). Deleted 752 lines of TypeScript (layout-engine.ts, project-manager.ts), added 846 lines of Python with Pydantic models (services/layout_engine.py, services/project_service.py, models/layout.py, models/project.py). Achieved 10-20x performance improvement via direct i3ipc library calls instead of shell commands. Established clear architectural separation: Python daemon handles all backend operations (file I/O, /proc reading, i3 IPC), TypeScript CLI handles only UI (argument parsing, display formatting). Added 8 new JSON-RPC methods for layout and project management. Zero user-facing changes - all CLI commands work identically.
