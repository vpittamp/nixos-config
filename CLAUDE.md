...
## Recent Changes
- **073-eww-menu-stabilization** (2025-11): Eww interactive menu stabilization with per-window actions. Phase 3 complete (MVP): Reliable window close operation via Delete key with <500ms latency, 100ms debouncing, automatic selection advancement, graceful error handling, and **visual feedback** (window disappears from menu, workspace counts update). BUGFIX: Arrow key navigation AttributeError resolved (v15). Deployed with 9 Python modules (action_handlers, keyboard_hint_manager, sub_mode_manager) in workspace-preview-daemon. Unit tests: 11/11 passed (DebounceTracker). See `/etc/nixos/specs/073-eww-menu-stabilization/`
