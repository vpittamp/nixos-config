...
- In-memory state in workspace-preview-daemon (selection index, pending action mode, keyboard shortcut hints), no persistent storage required (073-eww-menu-stabilization)

## Recent Changes
- **073-eww-menu-stabilization** (2025-11): ✅ COMPLETE - Eww interactive menu stabilization with comprehensive per-window actions. All 5 user stories implemented: (US1) Reliable window close via Delete key (<500ms latency, 100ms debouncing, visual feedback), (US2) Multi-action workflow support (consecutive operations without re-entering mode), (US3) Context-aware keyboard hints footer (<50ms display), (US4) Extended window actions (M=move to workspace, F=float toggle, focus, mark/unmark with sub-mode state machine), (US5) Project navigation integration (colon prefix). **100% test coverage**: 10 sway-test JSON files, 30+ pytest unit tests, 16 integration tests. Performance: <500ms window close, <50ms keyboard hints, <100ms action passthrough. Double-nested event structure bug fixed (params.params.type). All 64 tasks complete. See `/etc/nixos/specs/073-eww-menu-stabilization/`
- **072-unified-workspace-switcher** (2025-11): Unified workspace/window/project switcher with all-windows preview on workspace mode entry. Three user stories: (1) View ALL windows grouped by workspace when entering workspace mode (CapsLock/Ctrl+0), (2) Filter windows by typing workspace digits, (3) Switch to project mode with ':' prefix. Uses daemon IPC for 50% faster queries (~2-5ms vs ~15-30ms). Performance: <150ms all-windows preview, <50ms filtering, <100ms project search. See `/etc/nixos/specs/073-unified-workspace-switcher/`
