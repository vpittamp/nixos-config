...
- The legacy static `Secret-cluster-ryzen.yaml` and cluster ExternalSecret were removed. Do not recreate them. ArgoCD's `cluster-ryzen` is the agent mapping written by `argocd-agentctl`; Headlamp uses the separately staged `headlamp-cluster-ryzen` read-only kube credential.
