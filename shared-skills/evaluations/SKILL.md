...
- **⚠️ Ryzen capacity rule — multi-agent / parallel runs MUST be sequential.** Agent **inference/dispatch is NOT admission-controlled** (only eval TaskRuns are Kueue-gated). On the ryzen workstation, running multiple benchmark runs in parallel (e.g. 3 runs × conc 3 = 9 concurrent inference workflows) saturates the single `workflow-orchestrator` (`503 workflow_runtime_unavailable`, connection-pool exhaustion) and exhausts a worker node's RAM → **kubelet stops posting node status → `postgresql-0` + `workflow-orchestrator` (co-located, unprotected) cascade down**. Proven-safe envelope = **one run at a time, concurrency ≤ 3** (run `34XD3F61`/`cmp3seq-0519`). For `Compare agents`, launch the runs **sequentially** (deepseek → opus-4.7 → gpt-5.4), each gated on a fresh ryzen-health check (all nodes Ready + postgres/orchestrator Running) before launching the next; abort the sequence if a node flaps. The generalized admission-control + critical-pod-protection architecture is designed but **not yet implemented** (plan at `/home/vpittamp/.claude/plans/create-a-plan-to-hidden-kitten.md`); until it lands, sequential-only on ryzen is the operational rule.
- **Dev agent-runtime inference gap.** Dev's swe agents' inference path is not wired (no AgentRuntime CR / no openshell sandbox spawned) → a dev canary produces **no patches**; every instance grades `empty_patch`/`failed`. This is NOT an eval/Kueue/coordinator defect — the coordinator correctly writes empty predictions, continues to the evaluator, and the run reaches `completed` with structured `harness_result`. When validating a SWE-bench promotion on dev, verify the **eval/Kueue chain** (TaskRun+pod `benchmark-fast` labels, admitted Workloads, 0 OOM, run completed) — do NOT expect `resolved`. Wiring dev inference is a separate, untracked gap.
