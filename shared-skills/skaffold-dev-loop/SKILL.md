...
- **Workloads image pins use bare `name: <svc>` match-keys (all workflow-builder-system services).** The base `workloads/<svc>/manifests/kustomization.yaml` matches `name: <svc>` → `newName: ghcr.io/pittampalliorg/<svc>` + `newTag: git-<sha>`, with the Deployment carrying `image: <svc>:latest`. workflow-builder always used this bare form; `workflow-orchestrator`/`function-router`/`mcp-gateway` were flipped to it from the retired `gitea-ryzen.tail286401.ts.net/giteaadmin/<svc>` long-form in stacks PR #2435 (image-neutral — resolved image byte-identical). `commit-pin.sh` matches `name == <svc>` OR `name endswith /<svc>`, so it still handles any residual long-form name; `skaffold-status`/`doctor` no longer report `NAME-MISMATCH` for these services (`pinName` is the bare `<svc>`).
