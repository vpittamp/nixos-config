...
- **Outer loop** (`pnpm deploy:skaffold`) — Skaffold builds the prod multi-stage Dockerfile, pushes to `ghcr.io/pittampalliorg/<svc>:git-<sha>`, then a wrapper commit-pins the new tag on the `main` GitHub branch. For most Skaffold-owned services the wrapper edits the bare `stacks/main/.../workloads/<service>/manifests/kustomization.yaml` `newTag`. For **workflow-builder** + **workflow-mcp-server** the wrapper instead upserts the flat pins file `hub-spoke-appsets/release-pins/workflow-builder-images-ryzen.yaml`, and a stacks CI action renders that into a kustomize Component the workflow-builder manifests include (see the outer-loop section). Either way ryzen's LOCAL ArgoCD (`root-ryzen` tracks `main` directly) rolls the new image to ryzen on its next reconcile (no hub Source Hydrator, no Promoter on the ryzen lane).
...
- Each run: fetch + hard-reset to remote tip, then write the pin:
  ...
  - **`workflow-builder` + `workflow-mcp-server`**: instead of touching the manifests `newTag`, UPSERTS the FLAT pins file `hub-spoke-appsets/release-pins/workflow-builder-images-ryzen.yaml` (`images`/`imageRefs`/`digests`/`sourceShas`), commit, push. commit-pin does NOT render the Component locally — a stacks CI action does that on push (drift-proof; see below).
...
- After push (and, for workflow-builder/workflow-mcp-server, after the CI action renders the Component), ryzen's local ArgoCD (`root-ryzen` @ `main`) re-renders `overlays/ryzen` and applies the new image tag — no hub Source Hydrator, no `env/spokes-ryzen` (both retired for ryzen).
- **For workflow-builder + workflow-mcp-server only**: the bare `images:` block was DELETED from `workloads/workflow-builder/manifests/kustomization.yaml`; it now `components:`-includes a render-generated Component at `workloads/workflow-builder-ryzen-image/kustomization.yaml` carrying the workflow-builder + workflow-mcp-server pin (`newName`/`newTag`). A stacks CI action (`.github/workflows/render-ryzen-image.yml`) auto-renders that Component from the flat pins file on every push that touches it — via `scripts/gitops/render-workflow-builder-release-overlays.sh` with `WFB_RENDER_ENVS=ryzen` — and commits the regenerated Component (author `github-actions[bot]`). ryzen's workflow-builder Application sources `manifests/` directly, so the Component IS ryzen's effective pin. (stacks #2443/#2447, wfb #33.)
...
`commit-pin.sh` upserts the flat `release-pins/workflow-builder-images-ryzen.yaml` pins file on `main` and pushes; the stacks CI action then renders the kustomize Component (no manual render) and ryzen's local ArgoCD reconciles it. The command hard-refreshes the local `ryzen-workflow-builder` app. On the **NixOS 403** push failure, push the cached pin manually then hard-refresh (the CI action still runs automatically once the push lands):
...
> **Architecture note (why it's still a manual commit-pin trigger):** ryzen is the autonomous fast-lane — no Promoter, its pin decoupled from dev/staging so it can run a different/newer/test revision. As of 2026-06-04 (stacks #2443/#2447, wfb #33) the workflow-builder + workflow-mcp-server ryzen pin IS fully wired and automated *once you commit-pin*: commit-pin writes the flat `release-pins/workflow-builder-images-ryzen.yaml`, the stacks CI action `render-ryzen-image.yml` renders the `workflow-builder-ryzen-image` kustomize Component (which the workflow-builder manifests `components:`-include) and commits it, and ryzen's autonomous local ArgoCD reconciles `overlays/ryzen@main` (~30s). Validated end-to-end — the only manual step is firing commit-pin (the github-outer-loop deliberately doesn't touch ryzen's pin file). This supersedes the older "Phase 2c/2d not yet wired / ryzen still reads the bare manifests" note: Phase 2d is DONE.
