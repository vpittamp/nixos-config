...
4. For ryzen-only validation, repoint the relevant `packages/components/workloads/<image>/manifests/kustomization.yaml` to that GHCR tag and commit/merge to `main` (ryzen's local ArgoCD reconciles `main` directly; `deployment/scripts/ryzen-sync.sh` forces an immediate re-compare).
