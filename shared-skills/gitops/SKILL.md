...
Practical limits for `/workspaces/<slug>/benchmarks` parallelism. Dev has 5 Talos nodes × 8 CPU × 16 GiB, but only **2 are workload-eligible** (3 are control-plane tainted; 1 was `cilium.io/agent-not-ready` mid-2026-04). Tekton work pins to a single worker (`talos-eew-ya6`) via affinity-assistant PVC affinity, so all SWE-env builds compete for that node's 16 GiB.

Per-instance peak draw during inference (sum of agent-runtime pod + openshell-sandbox pod): **~4 GiB**. Per-run harness evaluator Job adds **~6–8 GiB peak** (evaluator container 8Gi limit + dind 4Gi limit, "standard" profile). Per-env-image build (first run only): 4Gi req / 6Gi limit, **lock-serialized** so only 1 buildah at a time across all parallel runs.
...
**Safe rule of thumb**: ≤ **5 active inference instances total** AND ≤ **2 active runs**. If env images aren't cached (first run hitting a new repo/version), drop to 1 run at a time until they cache (single-writer buildah lock). When you need real headroom, scale to 4 worker nodes — the 16 GiB / single-worker bottleneck is structural, not a tuning knob. Reuse runs against repo/version pairs you've already built (django-3.2, django-4.1, matplotlib-3.7, sympy-1.11, etc.) to skip env builds entirely.
