...
- **Hub Tekton owns the build plane**:
  ...
  - **Gitea/dev-image lane**: triggered by Gitea webhooks to `https://el-workflow-builds-hub.tail286401.ts.net`; builds on hub `hub-build-*` nodes, pushes to the **gitea-ryzen** registry, and commits `active-development/manifests/<image>/kustomization.yaml` changes to `gitea-ryzen/main`. This drives **ryzen** only, but it does not run Buildah on ryzen.
  - **SWE-bench inference image lane**: submitted by workflow-builder/swebench-coordinator preflight as `swe-env-<envSpecHash-prefix>` PipelineRuns on hub Tekton. These validate and pin repo/version/base images into `SWEBENCH_INFERENCE_ENVIRONMENTS_DIR` ConfigMap data. If a dev SWE-bench run sits `queued` while a `swe-env-*` PipelineRun exists on hub, the run is waiting for hub environment validation; do not look for Buildah pods on dev.
  - A workflow-builder app push that touches `services/dapr-agent-py` normally fires three runtime image builds: `dapr-agent-py-image-build`, `dapr-agent-py-sandbox-image-build`, and `dapr-agent-py-testing-sandbox-image-build`. It can also fire `workflow-builder-image-build` from the same commit. Watch all of them before deciding a rollout is complete.
...
|---|---|---|---|
| **ryzen** | `packages/components/active-development/manifests/<image>/kustomization.yaml` (`images:` block) | Hub Tekton Gitea/dev-image lane `workflow-builder-image-build`, triggered by Gitea webhook and scheduled on hub build nodes (commit subject: `chore(dev-images): deploy <image> <tag> to ryzen`) | `gitea-ryzen/main` (Gitea on ryzen) — **NOT pushed to GitHub** |
...
1. Which cluster? **ryzen only** → handled automatically by the hub Gitea/dev-image lane on the next push to the Gitea app repo. Nothing to do in stacks unless you're adding a new image or webhook/pipeline route.
...
2. Ryzen: watch the hub `workflow-builder-image-build` PipelineRun in `tekton-pipelines`, then verify both the declarative Deployment image and the actual serving pod on ryzen. DevSpace may leave the declarative `workflow-builder` Deployment at `replicas=0` while `workflow-builder-devspace` serves live traffic from synced source; inspect the live pod before assuming the image rollout is what users hit.
...
1. Confirm the workflow-builder commit includes the intended `services/swebench-evaluator/Dockerfile` pin or coordinator changes, and is pushed to `origin/main` for dev/staging and `gitea-ryzen/main` only if ryzen is part of the request.
...
- **Branch divergence is normal.** `origin/main` and `gitea-ryzen/main` drift because the hub GHCR/release lane commits to GitHub while the hub Gitea/dev-image lane commits ryzen tags to Gitea. Reconcile periodically via `runbooks/reconcile-branches.md`. Eleven files commonly conflict.
...
- **Skopeo mirror needs DNS to gitea-ryzen + GHCR org-write creds.** Prefer the normal hub build lanes first. For manual mirrors, run on ryzen with hub's `ghcr-push-credentials` secret, or run from a hub pod/job that maps `gitea-ryzen.tail286401.ts.net` to `gitea-ryzen-egress.tailscale.svc.cluster.local`. Off ryzen, SSH-wrap to ryzen because local DNS will not resolve `gitea-ryzen.tail286401.ts.net`. Agents should SSH-wrap when extracting production creds because local shell guards can trip on redirects. See `runbooks/mirror-image-gitea-to-ghcr.md`.
...
- **Hub pods cannot resolve `gitea-ryzen.tail286401.ts.net`.** Use the Tailscale **egress** service pattern (`gitea-ryzen-egress.tailscale.svc.cluster.local`) and add an `/etc/hosts` mapping before `git`, `curl`, `skopeo`, or webhook setup calls. Direct MagicDNS from hub pods will fail or hang.
