...
| Roll the `dapr-agent-py-sandbox` pin                     | One pin feeds the harness (`dapr-agent-harness`) and the executor image (`AGENT_RUNTIME_DEFAULT_IMAGE` on the BFF); wait for `rollout status` on `dapr-agent-harness`, `workflow-orchestrator`, and `workflow-builder` before a live proof |
