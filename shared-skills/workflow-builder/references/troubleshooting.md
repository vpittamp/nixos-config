...
| BFF returns `400 No active workspace` on POST | The BFF access JWT/login session has no active project | Select the intended workspace and refresh login auth, or use that workspace's Workflow MCP key and `save_workflow_script`. Do not stamp a project with a helper flag or SQL. |
...
| Symptom | Likely cause | Fix |
| --- | --- | --- |
| "Stop" returns **202 "stopping"** and the run lingers briefly | **By design (request/confirm)** — the stop persisted a `stop_requested_at` intent and the durable tree is still converging (e.g. a `terminate` blocked inside a long activity applies only when it yields). It does NOT flip DB / reap until Dapr is confirmed terminal. | Keep the UI/status client polling `GET …/stop/status` (→ `confirmDurableStop`). If the client closes, the intent remains until a later status read or explicit control-plane action; there is no terminal-reaper CronJob. A `durable/run` parent wedged awaiting a cross-app child can be force-finalized after `LIFECYCLE_WEDGE_FINALIZE_GRACE_SECONDS` once its child session is DB-terminated. |
...
| `/api/workflow-ops/*` returns 404 | The obsolete workflow-ops diagnostic surface was retired. | Use the scoped `/stop` and `/stop/status` routes. The unauthenticated orchestrator purge route and dead API-client methods were removed too. |
...
### Stuck durable / DB state

The old framing — "manual SQL resets and `scripts/*purge*` are the only cleanup" — is superseded by the Lifecycle Controller. Reach for manual SQL only as incident recovery after scoped stop/status/reset paths have run.
...
If a row remains stuck, read the explicit stop status, retry/confirm through the Lifecycle Controller, then use `mode:"reset"` if scoped cleanup is appropriate. Only then fall back to boundary-scoped `wfstate_state`/`agent_py_state` recovery after active runs and leases are zero. The Sandbox GC does not update durable or product rows.
...
| Benchmark run is terminal in DB/evaluator, but Dapr status still reports parent/session workflow `RUNNING` | The owning cancel/cleanup path did not finish durable closure, or its caller stopped polling the scoped stop-status route. | Reissue the authenticated benchmark/eval cancel action and follow its status surface. That owner invokes the Lifecycle Controller cascade, which terminates and purges per-session agent workflows before parent workflows. Do not edit DB or Dapr state first; there is no terminal-reaper backstop. If ordinary cleanup cannot converge, treat manual scoped state removal as an incident procedure only after proving there are no active runs or leases. |
...
| Symptom | Likely cause | Fix |
| --- | --- | --- |
| Legacy workflow does not appear in a workspace | Historical row lacks valid workspace ownership | Confirm through the scoped application API. Repair with an explicit migration/application command that calls the workflow-data port; do not backfill `project_id` directly. New saves through the BFF cannot create this state. |
| Workflow visible to one user, not their teammate | Workspace membership or role is missing | Repair membership through Workspace settings or the membership API; do not insert `project_members` rows manually. |
