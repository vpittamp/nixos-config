...
- `reset` — purge + force-delete state rows even if Dapr never confirmed terminal (dev clean-slate).

| Symptom | Likely cause | Fix |
| --- | --- | --- |
| "Stop" returns **409** / `{confirmed:false}` | **By design** — the controller is fail-closed: it did NOT confirm the durable tree is terminal, so it left DB rows + sandboxes intact rather than lie. | Retry the same call. If it keeps 409-ing, check the parent + per-session child workflow status (the cascade fans out terminate/purge **explicitly per per-session app-id** because the native Dapr recursive cascade doesn't cross task hubs) and the Sandbox CR / pod state, then escalate to `mode:"reset"`. |
