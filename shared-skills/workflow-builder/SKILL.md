...
| Run SWE-bench instances that must show in workflow-builder UI | Use the evaluations skill. Official SWE-bench runs use `/workspaces/<slug>/benchmarks`, `benchmark_*` tables, `swebench-coordinator`, and evaluator Jobs, not a normal SW 1.0 workflow row. |
...
- **Ryzen workstation: run one benchmark campaign at a time and trust the full-instance Kueue cap.** SWE-bench inference is now Kueue-aware for the sandbox + agent-host bundle (`BENCHMARK_KUEUE_INSTANCE_REQUEST_MODE=host-worker-composite`), but ryzen is still a finite workstation. Do not run parallel multi-agent benchmark load there. For a single run, requested concurrency can be higher than effective concurrency; `MPIlRkKWC7UdvHgwFQEiR` correctly selected 3 exact-ready instances but ran 2 at a time because live full-instance request headroom only fit two bundles.

- **SWE-bench Solver agents (2026-05-19): `deepseek-v4-pro-swebench`, `opus-4-7-swebench` (`anthropic/claude-opus-4-7`), `gpt-5-4-swebench` (`openai/gpt-5.4`), plus qwen3/kimi/foundry canaries — on ryzen + dev.** They're DB-defined clones (`agents`+`agent_versions`, only `config.modelSpec` swapped; valid modelSpecs = keys in `services/dapr-agent-py/src/effective_agent_config.py`). Pool agents (`runtime_app_id=agent-runtime-pool-coding`) need **NO AgentRuntime CR** (`kubectl get agentruntime` is empty; the static pool serves them). Eval is now Kueue-OOM-admission-controlled (`benchmark-fast`); env-spec-exact image gate + crash-callback live. See `evaluations` skill.
...
- **Dapr workflow termination is asynchronous.** `terminate` means "request shutdown"; it is not proof the instance is terminal. For `durable/run`, close the `dapr-agent-py` session workflow and every known turn child workflow before closing the parent `workflow-orchestrator` instance, then poll status until terminal/missing before purge. Do not mark DB `sessions`/`workflow_executions` terminal, delete sandboxes, or release benchmark leases until durable shutdown is confirmed. If not confirmed, leave resources in place and retry later.
...
- **Do not parameterize the stable system prompt unless the cache benefit is worth losing.** Dapr Conversation supports provider prompt cache retention / response cache settings, and provider usage may report cached tokens. Keep volatile values such as cwd, run id, session id, sandbox name, and workflow input out of the system/preset prefix when possible; put per-run data in the appended user prompt or workflow input so prompt-cacheable prefixes stay stable.
...
- **For SWE-bench capacity work, preserve Dapr history determinism.** If tracing, MLflow, hooks, or debug behavior must be suppressed, keep the workflow schedule the same and make the activity body no-op. Do not remove or conditionally skip a scheduled activity/child workflow after histories exist; a recent `emit_mlflow_node_span` schedule mismatch killed replay. Also keep `CLEANUP_STALE_ON_STARTUP` opt-in and benchmark-safe; terminal benchmark cleanup should terminate/poll/purge session/turn workflows first, then parent workflows, then release leases/sandboxes.

- **Dapr-agent custom activities use scoped names only.** With Dapr Agents 1.0.3, repo-owned `services/dapr-agent-py` custom workflow activities are registered and called through `self._activity_name(...)`. Do not restore the old bare-name fallback or register both names "for compatibility"; stale benchmark histories should be cancelled/cleaned/purged instead of keeping an ambiguous activity namespace.
...
- **SWE-bench concurrency is not just the launch slider.** Effective throughput is capped by full-instance Kueue capacity (sandbox pod + agent-host/session-host pod), runtime slots, per-sidecar Dapr workflow capacity, state-store pressure, global benchmark caps, sandbox headroom, model caps, coordinator pacing, and evaluator parallelism. For limit changes or "why are only N running?" questions, switch to the evaluations skill and read `references/swebench-concurrency.md`.
...
- **Don't roll workflow-orchestrator (or coordinator) images while a workflow is mid-run.** Dapr durable-task replay compares the in-process code's `call_activity` ordering to the persisted history. If a new image lands on the worker pod between yields, replay fails with `Sub-orchestration task #N failed: A previous execution called call_activity with ID=M, but the current execution doesn't have this action with this ID`. The run is dead even if no orchestrator code actually changed (Python module import order, dep updates, or activity-registration reordering can shift IDs). Wait for active runs to finish before pushing image bumps. Hit twice on 2026-04-30 — both failed runs had this error and a fresh run on the stable image worked end-to-end.

- **`Run Random` on the benchmarks instances page samples the suite first, then N rows.** The button uses a dropdown of `[1, 3, 5, 10, 25, 50, 100]` (default 3) gated by `filteredRowCount`. Implementation picks the suite (from `suiteFilter` or most-represented suite among visible rows), restricts to that suite's rows, then samples N — earlier code sampled across multiple suites and dropped non-dominant ones, so users requesting N got N-1 or fewer. If you change the launch flow, keep the suite-first ordering.

- **Benchmarks UI now has 6 Braintrust-adoption surfaces.** The run-detail page (`/workspaces/<slug>/benchmarks/runs/<runId>`) and the run-instance drawer have these analyst surfaces. Each is gated on its data being present, so empty states render naturally:
  - **Phase F regression strip** on the compare page — Welch's t-test + Fisher's exact p-values per metric vs the baseline run. `src/lib/server/benchmarks/regression.ts` (pure TS, no deps) + `metric-regression-strip.svelte`.
  - **Phase G scorer tiles + per-instance rows** — 4 scorers (`patch_files_overlap_gold`, `ran_tests_locally`, `edit_minimality` LLM-judge, `reasoning_quality` LLM-judge) run inline in `recomputeRunSummary`. Stored in `benchmark_run_instance_scores` with unique `(run_instance_id, scorer_name, scorer_version)`. Empty for already-completed runs (run on instance terminal, not retroactively).
  - **Phase H promote-to-dataset** popover on the drawer's Harness tab — POSTs to `/api/evaluations/datasets/[id]/rows-from-benchmark` and writes `origin_run_instance_id` + `origin_session_id` on the new row.
  - **Phase I trace tab** in the drawer — header strip + scorers panel + waterfall + annotation footer. Uses the existing spans endpoint plus `/api/benchmarks/runs/[runId]/instances/[instanceId]/scores` for scorer rows.
  - **Phase J cohort pivots** — `pivot()` helper at `$lib/benchmarks/cohort.ts` (NOT `$lib/server/...`, intentionally — Svelte components import it client-side). 5 dimensions × 8 measures dropdowns. `RunStats.cohortRows` is shipped from the server load.
  - **Phase K human annotations** — 4 verdicts (correct/incorrect/partial/unsure) + reasoning textarea. CRUD at `/api/benchmarks/runs/[runId]/instances/[instanceId]/annotations`. Aggregated as `RunStats.humanAnnotations.{counts, totalAnnotated, harnessDisagreement}` — disagreement counts when human verdict ∈ {correct, incorrect} contradicts harness pass/fail.
