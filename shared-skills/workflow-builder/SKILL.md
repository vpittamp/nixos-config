...
- **Sandbox templates are looked up by name in `SANDBOX_TEMPLATE_IMAGES_JSON`.** A workflow's `workspace_profile` step takes `with.sandboxTemplate: <name>`; the BFF resolves that name against the `SANDBOX_TEMPLATE_IMAGES_JSON` env var on the workflow-builder Deployment to get the actual image. Currently registered: `dapr-agent`, `default-sandbox`, `dapr-agent-xlsx`, `xlsx`, `code-eval`. Adding a new template = add the env var entry in stacks, build the matching `services/openshell-sandbox/environments/Dockerfile.<name>`, point stacks at the resulting image ref, and deliver the manifest change with `idpbuilder stacks sync` on ryzen. See the gitops skill for the GitOps cadence.
