...
- **⚠️ Ryzen workstation: agent inference/dispatch is NOT admission-controlled — run multi-agent/benchmark load SEQUENTIALLY only (≤3 concurrent).** Only SWE-bench *eval* TaskRuns are Kueue-gated. Parallel agent runs (≈9 concurrent inference workflows) saturate the single `workflow-orchestrator` and freeze a worker node → `postgresql-0`+orchestrator cascade. This is the highest-cost footgun on ryzen right now. Depth + recovery: `evaluations` skill "System State Update (2026-05-19)" and `idpbuilder-cluster-rebuild` "Workstation Capacity & Cascade Recovery". The generalized fix is planned, not implemented (`~/.claude/plans/create-a-plan-to-hidden-kitten.md`).
