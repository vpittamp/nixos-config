...
description: "Author, visualize, and debug SW 1.0 workflows for the workflow-builder app. Use for agent durable/run steps, trigger schemas, jq expressions, action slugs, workflow or agent MCP connections, ActivePieces piece MCP auth, canvas JSON, DB-backed workflow rows, failed workflow runs, silent agents, Dapr workflow sidecar readiness, pod 1/2 due to daprd, the boundary between workflows and Benchmarks/SWE-bench UI runs, and workflow-orchestrator/workspace-runtime/swebench-coordinator troubleshooting across PittampalliOrg/workflow-builder and PittampalliOrg/stacks."
---
