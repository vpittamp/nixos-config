...
- **Workflow-builder dev runs via Skaffold (or legacy DevSpace) file sync.** Don't start `pnpm dev` or spin up local containers. The going-forward dev loop is Skaffold — `pnpm dev:skaffold` for inner-loop HMR (file sync into a Skaffold-owned dev pod), `pnpm dev:skaffold:all` for the full 6-service set, `pnpm deploy:skaffold` for outer-loop GitOps commit-pin. See the [`skaffold-dev-loop`](../skaffold-dev-loop/SKILL.md) skill. The legacy [`devspace-quick-iteration`](../devspace-quick-iteration/SKILL.md) flow stays available for `fn-system` (Knative) and any service not migrated to Skaffold. The same caveat applies to both: pod env vars are baked at start time — when the BFF Deployment manifest changes (e.g. `AGENT_RUNTIME_DEFAULT_IMAGE`, `SANDBOX_TEMPLATE_IMAGES_JSON`), ArgoCD updates the standard ReplicaSet but a long-lived sync'd pod keeps the old env until Skaffold rebuilds or `devspace purge` runs.
...
- **Don't `pnpm dev`** in the workflow-builder repo. The canonical dev loop is Skaffold file sync into a Skaffold-owned dev pod (`pnpm dev:skaffold` — see [`skaffold-dev-loop`](../skaffold-dev-loop/SKILL.md)); legacy DevSpace is the fallback for `fn-system`.
