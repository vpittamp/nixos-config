{
  ...
  # Create a symlink at $out/bin/google-chrome (disabled by default to avoid collisions)
  withSymlink ? false,
}:
...
stdenvNoCC.mkDerivation (finalAttrs: {
  ...
  src = fetchurl {
    ...
    hash = "sha256-uTWOsENgUdH9PB7EdQm1hOvoqAxks8Vo09P/LmWTpfs=";
  };
  ...
  installPhase = ''
    ...
    substituteInPlace $out/share/applications/com.google.Chrome.${dist}.desktop \
      --replace-fail /usr/bin/google-chrome-${dist} $exe
    substituteInPlace $out/share/applications/google-chrome-${dist}.desktop \
      --replace-fail /usr/bin/google-chrome-${dist} $exe
    substituteInPlace $out/share/gnome-control-center/default-apps/google-chrome-${dist}.xml \
