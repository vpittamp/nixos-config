...
| Blocked means user action required and uses request sound | `/tmp/herdr/src/app/actions.rs:76-100`; `/tmp/herdr/src/app/actions.rs:193-203` | `notification_boundary_type = "user_input_required"` is the only external alert path; helper receives `I3PM_NOTIFY_SOUND=request` | Already covered; completion sound intentionally not mirrored | `tests/085-sway-monitoring-widget/test_monitoring_data.py::test_emit_user_input_notification_after_delay_once_for_claude` |
| Completion transitions can notify in Herdr | `/tmp/herdr/src/app/actions.rs:20-45`; `/tmp/herdr/src/app/actions.rs:122-160` | Plain completion/exlicit stop does not emit external visual or sound notification | Not applicable by product policy | `tests/085-sway-monitoring-widget/test_monitoring_data.py::test_emit_plain_completion_transition_does_not_spawn_notification` |
| Active focused pane suppresses redundant notifications | `/tmp/herdr/src/app/actions.rs:47-52`; `/tmp/herdr/src/app/actions.rs:2574-2586` | Current/focused sessions are acknowledged and suppress pending user-input notifications | Already covered | `tests/085-sway-monitoring-widget/test_monitoring_data.py::test_emit_user_input_notification_suppressed_for_focused_session`; daemon focus tests |
| Delayed notifications are rechecked and cancelled if state changes | `/tmp/herdr/src/app/actions.rs:2566-2629`; `/tmp/herdr/src/app/actions.rs:2632-2652`; `/tmp/herdr/src/app/actions.rs:2721-2758` | i3pm schedules action-required alerts, rechecks boundary marker before delivery, cancels/replaces on clear/change | Already covered | `tests/085-sway-monitoring-widget/test_monitoring_data.py::test_emit_user_input_notification_cancels_when_boundary_clears_before_delay`; `...changed_marker_replaces_pending_delay` |
| Toast delivery can be terminal/system/herdr/off | `/tmp/herdr/src/server/notifications.rs:9-18`; `/tmp/herdr/src/api/schema.rs:135-148` | Herdr `[ui.toast] delivery = "off"`; QuickShell only admits `i3pm-agent` action toasts while general toast max remains 0 | Implemented locally; broad Herdr delivery intentionally disabled | `tests/085-sway-monitoring-widget/test_ai_view_regressions.py::test_agent_action_toasts_bypass_general_toast_suppression_only_for_i3pm_agent` |
