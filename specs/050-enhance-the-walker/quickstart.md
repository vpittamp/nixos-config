...
Elephant keeps a history of all commands you run via the runner provider (`>` prefix) in `~/.cache/elephant/runner_history.gob`. To save frequently-used commands to your custom commands:

```bash
# After running a command via Walker (Meta+D → >git status → Return)
# Save it from the command line:
walker-cmd save "git status"
# Rofi prompts for name → Enter "git status" → Return
```
...
**Why manual save?** Walker's action system doesn't support custom shell commands, only predefined actions like "run" and "runterminal". However, the `walker-cmd` tool makes saving commands quick and easy from the terminal.

**Tip:** Keep a terminal window open to quickly save useful commands you discover while working!
