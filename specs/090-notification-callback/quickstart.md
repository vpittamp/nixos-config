...
**Quick Reference**: Return to Claude Code terminal from any project/workspace with one click or Ctrl+R

## TL;DR

When Claude Code finishes a long-running task and you're working in another project, click "Return to Window" (or press **Ctrl+R**) in the notification to instantly switch back to the exact terminal and tmux pane where Claude Code is waiting.

**Before**: Manually navigate back (10-15s): switch project → find workspace → locate terminal → enter correct tmux pane
**After**: Ctrl+R (< 2s): automated project switch + workspace focus + terminal focus + tmux pane selection
...
4. **Receive notification when Claude Code finishes**:
   ...
   📊 Activity: 5 bash, 3 reads

   📁 nixos-090-notification-callback

   Source: nixos-090:0

   [🖥️  Return to Window]  [Dismiss]
   ```

5. **Click "Return to Window" or press Ctrl+R**:
...
| Key | Action |
|-----|--------|
| **Ctrl+R** | Return to Claude Code terminal (same as clicking "Return to Window") |
| **Escape** | Dismiss notification without changing focus |
| **Enter** | Return to terminal (alternative to Ctrl+R) |
...
4. Press **Ctrl+R** → instantly jump to workspace 1 terminal
...
4. Press **Ctrl+R**: Back to `nixos-090` terminal instantly
...
4. Press **Ctrl+R**: Jump to workspace 1 terminal
...
### SwayNC Keybindings (Already Configured)
...
**No manual configuration needed** - keybindings automatically generated during home-manager activation.
...
**Symptom**: Clicking notification action or pressing Ctrl+R does nothing
...
# 5. Wait for notification, click "Return to Window" or press Ctrl+R
...
# Expected output:
# ✓ Same-project terminal focus after notification (1.2s)
```
...
Source: nixos-090:0
[Return to Window] ← Focuses Terminal 1
```
...
Notification: Claude Code Ready
Source: dotfiles:1
[Return to Window] ← Focuses Terminal 2
```
...
# Press Ctrl+R → Scratchpad terminal shows and focuses
```
...
Win+P → Select "nixos-089-cleanup"

# Claude Code finishes in nixos-090 project
# Notification arrives with "nixos-090:0" source info

# Press Ctrl+R → Project switcher reverses navigation back to nixos-090
```
...
# Press Ctrl+R → Switches back to hetzner-dev project, focuses SSH terminal
```
