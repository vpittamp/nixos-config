...
ShellRoot {
    ...
    function appScoringNormalize(value) {
        return String(value || "").toLowerCase().replace(/[^a-z0-9]+/g, " ").trim();
    }

    function appScoringCompact(value) {
        return String(value || "").toLowerCase().replace(/[^a-z0-9]+/g, "");
    }

    function appScoringExtractFilters(rawQuery) {
        var filters = { scope: null, workspace: null, monitor: null, pwa: false };
        var remaining = [];
        var tokens = rawQuery.split(/\s+/);
        for (var i = 0; i < tokens.length; i += 1) {
            var lower = tokens[i].trim().toLowerCase();
            if (lower === "@scoped" || lower === "scope:scoped") { filters.scope = "scoped"; continue; }
            if (lower === "@global" || lower === "scope:global") { filters.scope = "global"; continue; }
            if (lower === "@pwa" || lower === "pwa" || lower === "type:pwa") { filters.pwa = true; continue; }
            if (lower.indexOf("ws:") === 0) {
                var wsSuffix = lower.slice(3);
                if (/^\d+$/.test(wsSuffix)) { filters.workspace = parseInt(wsSuffix, 10); continue; }
            }
            if (lower.indexOf("monitor:") === 0) {
                var monSuffix = lower.split(":")[1];
                if (monSuffix === "primary" || monSuffix === "secondary" || monSuffix === "tertiary") { filters.monitor = monSuffix; continue; }
            }
            remaining.push(tokens[i]);
        }
        return { query: remaining.join(" ").trim(), filters: filters };
    }

    function appScoringMatchesFilters(app, appFilter, queryFilters) {
        var name = String(app.name || "").trim();
        var scope = String(app.scope || "").trim().toLowerCase();
        var workspace = (app.preferred_workspace !== undefined && app.preferred_workspace !== null) ? parseInt(app.preferred_workspace, 10) : null;
        if (isNaN(workspace)) { workspace = null; }
        var monitorRole = String(app.preferred_monitor_role || "").trim().toLowerCase();
        var isPwa = name.length > 4 && name.slice(-4) === "-pwa";

        if (appFilter === "scoped" && scope !== "scoped") { return false; }
        if (appFilter === "global" && scope !== "global") { return false; }
        if (appFilter === "pwa" && !isPwa) { return false; }
        if (appFilter === "workspace" && workspace === null) { return false; }

        if (queryFilters.scope && scope !== queryFilters.scope) { return false; }
        if (queryFilters.workspace !== null && workspace !== queryFilters.workspace) { return false; }
        if (queryFilters.monitor && monitorRole !== queryFilters.monitor) { return false; }
        if (queryFilters.pwa && !isPwa) { return false; }
        return true;
    }

    function appScoringScore(app, index, queryNorm, queryCompact, queryTokens) {
        if (!queryNorm) {
            return 1000 - Math.min(index, 999);
        }

        var name = String(app.name || "");
        var displayName = String(app.display_name || name);
        var description = String(app.description || "");
        var domain = String(app.pwa_domain || "");
        var aliases = Array.isArray(app.aliases) ? app.aliases : [];
        var matchDomains = Array.isArray(app.pwa_match_domains) ? app.pwa_match_domains : [];

        var nameWithoutSuffix = (name.length > 4 && name.slice(-4) === "-pwa") ? name.slice(0, -4) : name;
        var fields = [
            appScoringNormalize(displayName),
            appScoringNormalize(name),
            appScoringNormalize(nameWithoutSuffix),
            appScoringNormalize(description),
            appScoringNormalize(domain)
        ];
        for (var a = 0; a < aliases.length; a += 1) { fields.push(appScoringNormalize(aliases[a])); }
        for (var m = 0; m < matchDomains.length; m += 1) { fields.push(appScoringNormalize(matchDomains[m])); }

        var compactFields = [];
        for (var c = 0; c < fields.length; c += 1) {
            var cv = appScoringCompact(fields[c]);
            if (cv) { compactFields.push(cv); }
        }
        ...
        for (var f = 0; f < fields.length; f += 1) {
            if (fields[f]) {
                prefixValues.push(fields[f]);
                exactSet[fields[f]] = true;
            }
        }

        var score = 0;

        if (exactSet[queryNorm]) {
            score = Math.max(score, 320);
        }

        for (var p = 0; p < prefixValues.length; p += 1) {
            if (prefixValues[p].indexOf(queryNorm) === 0) { score = Math.max(score, 260); break; }
        }

        if (queryCompact) {
            for (var q = 0; q < compactFields.length; q += 1) {
                if (compactFields[q].indexOf(queryCompact) !== -1) { score = Math.max(score, 200); break; }
            }
        }

        for (var s = 0; s < prefixValues.length; s += 1) {
            if (prefixValues[s].indexOf(queryNorm) !== -1) { score = Math.max(score, 140); break; }
        }

        var tokenHits = 0;
        var exactTokenHits = 0;
        for (var t = 0; t < queryTokens.length; t += 1) {
            var matched = false;
            for (var v = 0; v < prefixValues.length; v += 1) {
                var words = prefixValues[v].split(" ");
                if (words.indexOf(queryTokens[t]) !== -1) {
                    exactTokenHits += 1;
                    tokenHits += 1;
                    matched = true;
                    break;
                }
                if (prefixValues[v].indexOf(queryTokens[t]) !== -1) {
                    tokenHits += 1;
                    matched = true;
                    break;
                }
        ...
        score += (tokenHits - exactTokenHits) * 12;
    ...
    function computeAppLauncherEntries(query, appFilter) {
        var raw = "";
        try {
            var xhr = new XMLHttpRequest();
            xhr.open("GET", "file://" + runtimeConfig.appRegistryPath, false);
            xhr.send();
        ...
        if (!raw || !raw.trim()) {
            try {
                ...
                xhr2.open("GET", "file://" + runtimeConfig.appRegistryFallbackPath, false);
                xhr2.send();
                raw = xhr2.responseText || "";
            } catch (_e2) {}
        }
        ...
        var applications = Array.isArray(data.applications) ? data.applications : [];
        if (!applications.length) {
            return [];
        }

        var filterNorm = String(appFilter || "all").trim().toLowerCase();
        var extracted = appScoringExtractFilters(query || "");
        var filteredQuery = extracted.query;
        var queryFilters = extracted.filters;
        var queryNorm = appScoringNormalize(filteredQuery);
        var queryCompact = appScoringCompact(filteredQuery);
        var queryTokens = queryNorm ? queryNorm.split(/\s+/).filter(function(t) { return !!t; }) : [];

        var limit = 20;
        var minScore = 20;
        var desktopDir = runtimeConfig.appDesktopDir;
        var scored = [];

        for (var i = 0; i < applications.length; i += 1) {
            var app = applications[i];
            if (!app || typeof app !== "object") { continue; }
            if (!appScoringMatchesFilters(app, filterNorm, queryFilters)) { continue; }

            var appScore = appScoringScore(app, i, queryNorm, queryCompact, queryTokens);
            if (appScore < minScore) { continue; }

            var appName = String(app.name || "").trim();
            var displayName = String(app.display_name || appName).trim();
            if (!appName || !displayName) { continue; }

            var workspace = (app.preferred_workspace !== undefined && app.preferred_workspace !== null) ? parseInt(app.preferred_workspace, 10) : null;
            if (isNaN(workspace)) { workspace = null; }
            var monitorRole = String(app.preferred_monitor_role || "").trim();
            var aliases = Array.isArray(app.aliases) ? app.aliases : [];
            var state = [];
            var isPwa = appName.length > 4 && appName.slice(-4) === "-pwa";
            if (isPwa) { state.push("pwa"); }
            var scope = String(app.scope || "").trim().toLowerCase();
            if (scope) { state.push(scope); }
            if (workspace !== null) { state.push("workspace-pinned"); }

            var identifier = desktopDir + "/" + appName + ".desktop";

            var subtextParts = [];
            var desc = String(app.description || "").trim();
            if (desc) { subtextParts.push(desc); }
            if (workspace !== null) { subtextParts.push("WS" + String(workspace)); }
            var rawScope = String(app.scope || "").trim();
            if (rawScope) { subtextParts.push(rawScope); }

            var badges = [];
            if (isPwa) { badges.push({ label: "PWA", tone: "teal" }); }
            if (scope === "scoped") { badges.push({ label: "Scoped", tone: "orange" }); }
            else if (scope === "global") { badges.push({ label: "Global", tone: "blue" }); }
            if (workspace !== null) { badges.push({ label: "WS" + String(workspace), tone: "violet" }); }

            var cleanAliases = [];
            for (var ai = 0; ai < aliases.length; ai += 1) {
                var av = String(aliases[ai]).trim();
                if (av) { cleanAliases.push(av); }
            }
            ...
            scored.push({
                ...
                entry: {
                    ...
                    preferred_monitor_role: monitorRole || null,
        ...
        scored.sort(function(a, b) {
            ...
            var aText = a.entry.text.toLowerCase();
            var bText = b.entry.text.toLowerCase();
        ...
        var result = [];
        var count = Math.min(scored.length, limit);
        for (var r = 0; r < count; r += 1) {
            result.push(scored[r].entry);
        }
        return result;
    }
    ...
    function restartLauncherQuery() {
        ...
        if (launcherMode === "apps") {
            ...
            try {
                setLauncherEntries(computeAppLauncherEntries(launcherQuery, launcherAppFilter));
            } catch (err) {
                console.warn("launcher.apps.inline:", err);
                setLauncherEntries([]);
                launcherError = "Unable to load application registry";
            }
            return;
        }
