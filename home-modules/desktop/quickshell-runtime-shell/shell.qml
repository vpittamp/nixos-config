...
ShellRoot {
    ...
    function sessionGroupTitle(group) {
        return shortProject(group.project_name || "global");
    }

    function sessionGroupSubtitle(group) {
        const bits = [];
        const title = stringOrEmpty(group.window_title);
        const host = stringOrEmpty(group.host_name);
