...
def project_names_match(
    ...
    candidate_project: Optional[str],
) -> bool:
    ...
    return (
        re.sub(r"[^a-z0-9]+", "-", preferred).strip("-")
        == re.sub(r"[^a-z0-9]+", "-", candidate).strip("-")
    )
...
class IPCServer:
    ...
    def _assistant_desktop_window_score(
        self,
        *,
        query: str,
        window: Dict[str, Any],
        ...
        search_text = self._assistant_desktop_window_search_text(window)
    ...
    def _resolve_assistant_window_target(
        self,
        *,
        ...
        for item in tracked_windows:
            score = self._assistant_desktop_window_score(
    ...
    def _assistant_desktop_app_score(self, query: str, app: RegistryApp) -> float:
        normalized_query = str(query or "").strip().lower()
        if not normalized_query:
            return 0.0
        fields = [
    ...
    def _serialize_registry_app(self, app: RegistryApp) -> Dict[str, Any]:
        return {
            "name": app.name,
    ...
    def _resolve_assistant_app_target(self, query: str) -> Dict[str, Any]:
        normalized_query = str(query or "").strip()
        if not normalized_query:
            return {"matched": False, "ambiguous": False, "app": None, "candidates": []}

        self.registry_loader.ensure_current()
        apps = self.registry_loader.list_all()
        ranked: List[Tuple[float, RegistryApp]] = []
        for app in apps:
            score = self._assistant_desktop_app_score(normalized_query, app)
            if score > 0:
    ...
    def _assistant_desktop_processes(self, limit: int = 8) -> List[Dict[str, Any]]:
    ...
    def _assistant_desktop_workspace_summary(
        self,
        runtime_snapshot: Dict[str, Any],
    ...
    def _build_assistant_desktop_snapshot(
        self,
        runtime_snapshot: Dict[str, Any],
        *,
        ...
        processes = self._assistant_desktop_processes(process_limit) if include_processes else []
        workspace_summary = self._assistant_desktop_workspace_summary(runtime_snapshot, focused_window)
        runtime_summary = {
            "daemon_health": "ok",
            "desktop_revision": int(self._snapshot_version),
    ...
    async def _assistant_desktop_snapshot(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        return self._build_assistant_desktop_snapshot(
            runtime_snapshot,
    ...
    async def _assistant_desktop_resolve_action(
        ...
        *,
        ...
        desktop_snapshot = self._build_assistant_desktop_snapshot(
            runtime_snapshot,
        ...
        if action_kind == "get_desktop_context":
            ...
            if query:
                ...
                windows = [
                    ...
                    if query_lower in self._assistant_desktop_window_search_text(item)
                ]
            ...
            processes = self._assistant_desktop_processes(int(params.get("limit") or params.get("process_limit") or 8))
            ...
            resolved = self._resolve_assistant_window_target(
            ...
            resolved = self._resolve_assistant_app_target(query)
    ...
    async def _assistant_desktop_preview(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        payload = dict(params or {})
        runtime_snapshot = await self._runtime_snapshot(payload)
        preview = await self._assistant_desktop_resolve_action(payload, runtime_snapshot=runtime_snapshot)
    ...
    async def _assistant_desktop_execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        payload = dict(params or {})
        runtime_snapshot = await self._runtime_snapshot(payload)
        resolved = await self._assistant_desktop_resolve_action(payload, runtime_snapshot=runtime_snapshot)
        ...
        after_snapshot = await self._assistant_desktop_snapshot({
            "include_processes": bool(payload.get("include_processes", False)),
            "process_limit": int(payload.get("process_limit") or 8),
        })
    ...
    async def _reconcile_launch_runtime_status(
        ...
        *,
        ...
        if not launch_key:
            return {}
        status = self._read_launch_status(launch_key)
        if not status:
            return {}
        if str(status.get("status") or "").strip() == "failed":
            return status
        spec = self._read_launch_spec(launch_key)
        if not spec:
            return status
        ...
        *,
        ...
        try:
            ...
            for attempt in range(max(int(attempts), 1)):
                result = await self._reconcile_launch_runtime_status(
                    launch_id,
    ...
    def _schedule_launch_reconcile(
        ...
        *,
        ...
        self._launch_reconcile_tasks[launch_key] = asyncio.create_task(
            self._run_launch_reconcile_loop(
                launch_key,
                anchor_bound=anchor_bound,
                attempts=attempts,
                delay_s=delay_s,
    ...
    async def _mark_launch_window_bound(
        ...
        *,
        ...
        result = await self._reconcile_launch_runtime_status(launch_id, anchor_bound=True)
        if not result:
            return {}
        ...
        if not bool(probe.get("exists", False) and probe.get("healthy", False)):
            self._schedule_launch_reconcile(launch_id, anchor_bound=True, attempts=25, delay_s=0.2)
            return result
        return self._write_launch_status(
            launch_id=launch_id,
            status="running",
    ...
    async def _mark_launch_window_closed(self, window_info: Any) -> Dict[str, Any]:
        ...
        if not launch_id:
            return {}
        result = await self._reconcile_launch_runtime_status(launch_id, anchor_bound=False)
        if str(result.get("status") or "").strip() not in {"reusable_headless", "failed"}:
            self._schedule_launch_reconcile(launch_id, anchor_bound=False, attempts=20, delay_s=0.2)
        return result
...
"""
    ...
    def _build_herdr_spaces(
        ...
        for session in herdr_sessions:
            ...
            if self._herdr_agent_status_rank(candidate_status) > self._herdr_agent_status_rank(current_status):
