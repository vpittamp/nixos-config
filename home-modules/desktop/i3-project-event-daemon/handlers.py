...
async def on_window_new(
    ...
    try:
        ...
        # Feature 101: Per-project scoped_classes are deprecated
        # All window classification now uses global app_classification only
        ...
        if is_scratchpad_terminal:
            ...
            if window_env and window_env.context_key:
                ...
                if (
                    str(window_env.connection_key or "").strip()
...
async def on_window_mark(
    ...
    try:
        ...
        # Format: SCOPE:PROJECT_NAME:WINDOW_ID (where SCOPE is "scoped" or "global")
        ...
        if project_marks:
            ...
            project_name = extract_project_from_mark(project_marks[0], window_id)
...
async def on_output(
    ...
    Observational handler: detects connect/disconnect/profile_changed transitions
    via OutputEventService and records them in the event buffer. Workspace
    redistribution on profile change is handled by
    monitor_profile_service._redistribute_after_profile_change (atomic with the
    profile switch itself), not by this handler.
