...
def test_herdr_service_reports_stale_remote_bridge_windows():
    service = HerdrService(
        notify_state_change=lambda event_type: asyncio.sleep(0),
        invalidate_snapshot_cache=lambda: None,
    )
    ...
    stale = service.stale_remote_bridge_windows(runtime_snapshot, [])
...
def test_herdr_service_bridge_diagnostics_reports_matched_remote_window():
    service = HerdrService(
        notify_state_change=lambda event_type: asyncio.sleep(0),
        invalidate_snapshot_cache=lambda: None,
    )
    ...
    diagnostics = service.bridge_diagnostics(runtime_snapshot, sessions)

    assert len(diagnostics) == 1
    assert diagnostics[0]["matched_session_key"] == "session-remote"
    assert diagnostics[0]["mismatch_reason"] == ""
    assert diagnostics[0]["remote_tmux_pane"] == "%0"
