{ config, pkgs, lib, osConfig ? {}, ... }:
{
  programs.bash = {
    ...
    initExtra = ''
      ...
      # Fix terminal resize in iTerm2/Terminal.app
      # Force update of LINES and COLUMNS on window resize
      # This ensures terminal content resizes properly
      if [[ "$OSTYPE" == "darwin"* ]]; then
        # Set up SIGWINCH trap to update terminal size
        trap 'eval $(resize -s 2>/dev/null || true)' SIGWINCH
        # Also update on each command prompt
        PROMPT_COMMAND="update_terminal_size; ''${PROMPT_COMMAND:+$PROMPT_COMMAND}"
        update_terminal_size() {
          shopt -s checkwinsize
          (:)  # Dummy command to trigger LINES/COLUMNS update
        }
      fi
