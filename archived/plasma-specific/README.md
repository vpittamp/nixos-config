...
### home-modules/desktop/generated/
Plasma rc2nix generated configuration files:
- plasma-rc2nix.nix - Exported Plasma settings snapshot
- plasma-rc2nix.nix.backup - Previous snapshot backup
- Generated by scripts/plasma-rc2nix.sh

**Archived because**: No longer tracking Plasma configuration snapshots.

### home-modules/desktop/pwas/
PWA window rules and activity assignment system:
- data.nix - PWA definitions with activity assignments
- generator.nix - Script generator for runtime patching of KWin rules
- window-rules.nix - KWin window rule generator for PWAs
- Integrated PWAs with KDE Activities system

**Archived because**: i3wm doesn't use KWin window rules. PWAs still work but use simple .desktop files.

### home-modules/desktop/browser-window-rules.nix
KWin window rules for browsers and applications:
- Firefox, Chromium, GitKraken rules
- K9s and Headlamp custom rules
- Activity assignment via KWin

**Archived because**: i3wm doesn't use KWin window rules for activity assignment.

### home-modules/desktop/monitoring-desktop-widgets.nix
KDE Plasma desktop widgets for system monitoring:
- Setup script for monitoring activity widgets
- System Monitor, CPU, Memory, Network widgets
- Activity-specific widget placement

**Archived because**: i3wm uses i3status for system monitoring instead of KDE widgets.

### home-modules/desktop/speech-to-text-shortcuts.nix
KDE global shortcuts for speech-to-text:
- Meta+Alt+D - Toggle nerd-dictation
- Meta+Alt+C - Speech to clipboard (10s recording)
- KDE hotkey configuration via programs.plasma
